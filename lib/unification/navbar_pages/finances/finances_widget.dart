...
class _FinancesWidgetState extends State<FinancesWidget> {
  ...
  static const Color _inverseSurface = Color(0xFF213145);
            ...
            Row(
              ...
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: isTrendError ? _errorContainer : _surfaceContainerHigh,
                ...
                Row(
                  children: ['1W', '1M', '1Y'].map((period) {
                    ...
                    return Padding(
                      ...
                      child: InkWell(
                        onTap: () => safeSetState(() => _selectedPeriod = period),
                        child: Container(
                          padding: const EdgeInsets.symmetric(horizontal: 12.0, vertical: 4.0),
                          decoration: BoxDecoration(
                            color: isSelected ? _clinicalBlue : _surfaceContainerLow,
                            borderRadius: BorderRadius.circular(6.0),
                            boxShadow: isSelected
                                ? [BoxShadow(color: _clinicalBlue.withValues(alpha: 0.2), blurRadius: 4.0, offset: Offset(0, 2))]
                                : null,
                          ),
                          child: Text(
                            period,
            ...
            Expanded(
              child: Container(
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(8.0),
                  border: Border.all(color: _outlineVariant.withValues(alpha: 0.3), width: 1.0),
                ),
                child: ClipRRect(
                  borderRadius: BorderRadius.circular(8.0),
                  child: CustomPaint(
                    painter: _RevenueChartPainter(
                      ...
                      gridColor: _outlineVariant.withValues(alpha: 0.1),
    ...
    return AuthUserStreamWidget(
      builder: (context) => StreamBuilder<List<FinanceRecord>>(
        ...
        builder: (context, snapshot) {
          ...
          final accountsReceivable = revenue > 0 ? (revenue - netProfit).abs() : 0.0;
    ...
    return StreamBuilder<List<UserRecord>>(
      stream: queryUserRecord(
        queryBuilder: (userRecord) => userRecord.where(
          'approved_by_duniya', isEqualTo: false),
      ),
      builder: (context, snapshot) {
        if (!snapshot.hasData) {
          return Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(12.0),
              border: Border.all(color: const Color(0xFFE2E8F0).withValues(alpha: 0.8), width: 1.0),
              boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: 0.04), blurRadius: 20.0, offset: const Offset(0, 4))],
            ),
            child: Center(child: SpinKitRing(color: const Color(0xFF9900FF), size: 32.0)),
          );
        }

        final pendingUsers = snapshot.data!;
        final pendingPharmacies = pendingUsers.where((u) => u.accountType == 'Pharmacy').toList();
        final pendingDuniyaUsers = pendingUsers.where((u) => u.accountType == 'Duniya' || u.accountType.isEmpty).toList();
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border(bottom: BorderSide(color: _outlineVariant.withValues(alpha: 0.3), width: 1.0)),
                ),
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Row(
                      children: [
                        Container(
                          ...
                          decoration: BoxDecoration(color: const Color(0xFFF3F0FF), borderRadius: BorderRadius.circular(10.0)),
                          child: Icon(Icons.pending_actions, size: 20.0, color: const Color(0xFF9900FF)),
                        ),
                        const SizedBox(width: 12.0),
                        Text('Pending Approvals', style: TextStyle(fontFamily: 'Satoshi', fontSize: 18.0, fontWeight: FontWeight.w600, letterSpacing: -0.01, height: 1.5, color: _onSurface)),
                      ],
                    ),
                    Row(
                      children: [
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 12.0, vertical: 4.0),
                          decoration: BoxDecoration(color: const Color(0xFFF3F0FF), borderRadius: BorderRadius.circular(9999.0)),
                          child: Text('${pendingPharmacies.length} Pharmacies', style: TextStyle(fontFamily: 'Satoshi', fontSize: 11.0, fontWeight: FontWeight.w600, color: const Color(0xFF9900FF))),
                        ),
                        const SizedBox(width: 8.0),
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 12.0, vertical: 4.0),
                          decoration: BoxDecoration(color: const Color(0xFFFEF9C3), borderRadius: BorderRadius.circular(9999.0)),
                          child: Text('${pendingDuniyaUsers.length} Duniya Users', style: TextStyle(fontFamily: 'Satoshi', fontSize: 11.0, fontWeight: FontWeight.w600, color: const Color(0xFF854D0E))),
                        ),
                      ],
                    ),
                  ],
              ...
              if (pendingUsers.isEmpty)
                Container(
                  ...
                  child: Center(
                    child: Column(
                      children: [
                        Icon(Icons.check_circle_outline, size: 48.0, color: const Color(0xFF16A34A).withValues(alpha: 0.5)),
                        const SizedBox(height: 12.0),
                        Text('All accounts are approved', style: TextStyle(fontFamily: 'Satoshi', fontSize: 15.0, fontWeight: FontWeight.w500, color: _onSurfaceVariant)),
                        const SizedBox(height: 4.0),
                        Text('No pending approvals at this time', style: TextStyle(fontFamily: 'Satoshi', fontSize: 13.0, color: _onSurfaceVariant.withValues(alpha: 0.7))),
                      ],
                    ),
                  ),
                )
                ...
                ...pendingUsers.map((user) {
                  ...
                  return Container(
                    ...
                    decoration: BoxDecoration(border: Border(bottom: BorderSide(color: _outlineVariant.withValues(alpha: 0.2), width: 1.0))),
                    child: Row(
                      children: [
                        Container(
                          width: 40.0, height: 40.0,
                          decoration: BoxDecoration(color: isPharmacy ? const Color(0xFFF3F0FF) : const Color(0xFFFEF9C3), borderRadius: BorderRadius.circular(12.0)),
                          child: Center(child: Text((user.displayName.isNotEmpty ? user.displayName : user.email).substring(0, 1).toUpperCase(), style: TextStyle(fontFamily: 'Satoshi', fontSize: 16.0, fontWeight: FontWeight.w700, color: isPharmacy ? const Color(0xFF9900FF) : const Color(0xFF854D0E)))),
                        ),
                        const SizedBox(width: 16.0),
                        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Text(user.displayName.isNotEmpty ? user.displayName : user.email, style: TextStyle(fontFamily: 'Satoshi', fontSize: 14.0, fontWeight: FontWeight.w600, color: _onSurface)),
                          const SizedBox(height: 2.0),
                          Text(user.email, style: TextStyle(fontFamily: 'Satoshi', fontSize: 12.0, color: _onSurfaceVariant)),
                        ])),
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 12.0, vertical: 4.0),
                          decoration: BoxDecoration(color: isPharmacy ? const Color(0xFFF3F0FF) : const Color(0xFFFEF9C3), borderRadius: BorderRadius.circular(9999.0)),
                          child: Text(isPharmacy ? 'Pharmacy' : 'Duniya', style: TextStyle(fontFamily: 'Satoshi', fontSize: 11.0, fontWeight: FontWeight.w600, color: isPharmacy ? const Color(0xFF9900FF) : const Color(0xFF854D0E))),
                        ),
                        const SizedBox(width: 12.0),
                        ElevatedButton(
                          onPressed: () async { await user.reference.update({'approved_by_duniya': true}); },
                          style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF9900FF), foregroundColor: Colors.white, elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(9999.0)), padding: const EdgeInsets.symmetric(horizontal: 16.0, vertical: 6.0)),
                          child: Text('Approve', style: TextStyle(fontFamily: 'Satoshi', fontSize: 12.0, fontWeight: FontWeight.w600)),
                        ),
                      ],
                    ),
                  );
...
class _RevenueChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Generate sample data points if no real data
    ...
    try {
      ...
      if (months.isNotEmpty && totals.isNotEmpty && months.length == totals.length) {
    ...
    if (dataPoints.isEmpty) {
      dataPoints = [250, 320, 280, 450, 380, 520, 480, 620, 580, 720, 650, 800];
    ...
    final normalizedData = dataPoints.map((v) => range > 0 ? (v - minVal) / range : 0.5).toList();
    ...
    for (int i = 0; i <= 4; i++) {
      ...
      final tp = TextPainter(text: textSpan, textDirection: dartui.TextDirection.ltr);
      tp.layout();
      tp.paint(canvas, Offset(chartPadding.left - tp.width - 8, y - tp.height / 2));
    }
    ...
    for (int i = 0; i < labels.length; i++) {
      final x = chartPadding.left + (chartWidth * i / (labels.length - 1).clamp(1, labels.length));
      final textSpan = TextSpan(text: labels[i], style: xLabelStyle);
      final tp2 = TextPainter(text: textSpan, textDirection: dartui.TextDirection.ltr);
      tp2.layout();
      tp2.paint(canvas, Offset(x - tp2.width / 2, size.height - chartPadding.bottom + 10));
    }
    ...
    for (int i = 0; i < normalizedData.length; i++) {
      final x = chartPadding.left + (chartWidth * i / (normalizedData.length - 1));
      final y = chartPadding.top + chartHeight - (chartHeight * normalizedData[i] * 0.85);
    ...
    final rect = Rect.fromLTWH(chartPadding.left, chartPadding.top, chartWidth, chartHeight);
    ...
    if (points.isNotEmpty) {
      ...
      // Outer glow
      canvas.drawCircle(lastPoint, 12.0, Paint()..color = primaryColor.withValues(alpha: 0.1));
      // Inner dot
      canvas.drawCircle(lastPoint, 6.0, Paint()..color = primaryColor);

      // Tooltip
      final tooltipText = dataPoints.length > 0 ? _formatVal(dataPoints.last) : '';
      ...
      final tp3 = TextPainter(text: tooltipSpan, textDirection: dartui.TextDirection.ltr);
      tp3.layout();
